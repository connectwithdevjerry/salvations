 ...
 * EVERYTHING the workspace has connected — first-party servers and every
 * enabled integration — because that is what "create an agent" promises: it
 * comes with memory, knowledge, its conversation and every tool the workspace
 * has, and a tool connected next week is its tool too without anybody editing
 * the agent. A narrowed list is the exception somebody chose on purpose.
 *
...
export async function agentSnapshotFor<T extends { capabilityBindings: readonly Attachment[] }>(
  ...
  const enabled = await bindingSource(database, workspaceId).listEnabled(workspaceId);
