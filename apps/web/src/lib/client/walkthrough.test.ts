...
describe('stepsFor', () => {
  ...
  it('points everyone at the workspace button, and tells owners it opens the dashboard', () => {
    const step = (admin: boolean) => stepsFor({ workspaceId: 'ws_1', admin }).find((s) => s.id === 'workspace');
    expect(step(true)?.target).toBe('workspace');
    expect(step(true)?.body).toContain('admin dashboard');
    expect(step(false)?.target).toBe('workspace');
    expect(step(false)?.body).toContain('Settings');
    const ids = stepsFor({ workspaceId: 'ws_1' }).map((s) => s.id);
    expect(ids.indexOf('workspace')).toBe(ids.length - 2);
  });
